...
pub mod remote_file_service_server {
    ...
    #[async_trait]
    pub trait RemoteFileService: std::marker::Send + std::marker::Sync + 'static {
    ...
    #[derive(Debug)]
    pub struct RemoteFileServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> RemoteFileServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T> Clone for RemoteFileServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "sift.remote_files.v1.RemoteFileService";
    impl<T> tonic::server::NamedService for RemoteFileServiceServer<T> {
