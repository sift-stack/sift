...
pub mod user_attributes_service_server {
    ...
    #[async_trait]
    pub trait UserAttributesService: std::marker::Send + std::marker::Sync + 'static {
    ...
    #[derive(Debug)]
    pub struct UserAttributesServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> UserAttributesServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T> Clone for UserAttributesServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "sift.user_attributes.v1.UserAttributesService";
    impl<T> tonic::server::NamedService for UserAttributesServiceServer<T> {
