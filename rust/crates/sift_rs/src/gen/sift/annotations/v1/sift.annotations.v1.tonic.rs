...
pub mod annotation_service_server {
    ...
    #[async_trait]
    pub trait AnnotationService: std::marker::Send + std::marker::Sync + 'static {
    ...
    #[derive(Debug)]
    pub struct AnnotationServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> AnnotationServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T> Clone for AnnotationServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "sift.annotations.v1.AnnotationService";
    impl<T> tonic::server::NamedService for AnnotationServiceServer<T> {
