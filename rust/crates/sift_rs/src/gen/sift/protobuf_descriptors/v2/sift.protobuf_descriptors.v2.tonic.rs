...
pub mod protobuf_descriptor_service_server {
    ...
    #[async_trait]
    pub trait ProtobufDescriptorService: std::marker::Send + std::marker::Sync + 'static {
    ...
    #[derive(Debug)]
    pub struct ProtobufDescriptorServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> ProtobufDescriptorServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T> Clone for ProtobufDescriptorServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "sift.protobuf_descriptors.v2.ProtobufDescriptorService";
    impl<T> tonic::server::NamedService for ProtobufDescriptorServiceServer<T> {
