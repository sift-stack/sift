...
pub mod user_defined_function_service_server {
    ...
    #[async_trait]
    pub trait UserDefinedFunctionService: std::marker::Send + std::marker::Sync + 'static {
    ...
    #[derive(Debug)]
    pub struct UserDefinedFunctionServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> UserDefinedFunctionServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T> Clone for UserDefinedFunctionServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "sift.user_defined_functions.v1.UserDefinedFunctionService";
    impl<T> tonic::server::NamedService for UserDefinedFunctionServiceServer<T> {
