...
*/
    ...
    ///
    #[derive(Debug)]
    pub struct TestReportServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> TestReportServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T> Clone for TestReportServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "sift.test_reports.v1.TestReportService";
    impl<T> tonic::server::NamedService for TestReportServiceServer<T> {
