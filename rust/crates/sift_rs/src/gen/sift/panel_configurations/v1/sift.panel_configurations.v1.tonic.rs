...
pub mod panel_configuration_service_server {
    ...
    #[async_trait]
    pub trait PanelConfigurationService: std::marker::Send + std::marker::Sync + 'static {
    ...
    #[derive(Debug)]
    pub struct PanelConfigurationServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> PanelConfigurationServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T> Clone for PanelConfigurationServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "sift.panel_configurations.v1.PanelConfigurationService";
    impl<T> tonic::server::NamedService for PanelConfigurationServiceServer<T> {
