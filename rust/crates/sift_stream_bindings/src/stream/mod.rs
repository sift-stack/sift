...
impl SiftStreamPy {
    ...
    pub fn send_requests(&self, py: Python, requests: Vec<request::IngestWithConfigDataStreamRequestPy>) -> PyResult<Py<PyAny>> {
        let inner = self.inner.clone();
    ...
    pub fn add_new_flows(&self, py: Python, flow_configs: Vec<FlowConfigPy>) -> PyResult<Py<PyAny>> {
        let inner = self.inner.clone();

        let flow_configs = flow_configs.into_iter()
            .map(|cfg| cfg.into())
