...
use sift_rs::{
    ...
    data_imports::v2::{
        ParquetDataColumn, ParquetFlatDatasetConfig,
...
pub fn detect_flat_dataset_config(file: &File) -> Result<ParquetFlatDatasetConfig> {
    ...
    Ok(ParquetFlatDatasetConfig { time_column, data_columns })
}
...
fn arrow_type_to_channel_data_type(dt: &DataType) -> Option<ChannelDataType> {
  ...
  DataType::FixedSizeBinary(_) => Some(ChannelDataType::Bytes),
...
fn detect_time_format(dt: &DataType) -> Option<TimeFormat>{
  ...
  Some(TimeFormat::AbsoluteUnixMicroseconds),
          DataType::Timestamp(TimeUnit::Nanosecond, _) =>
  Some(TimeFormat::AbsoluteUnixNanoseconds),
          DataType::Time32(TimeUnit::Second) =>
  Some(TimeFormat::RelativeSeconds),
          DataType::Time32(TimeUnit::Millisecond) =>
  Some(TimeFormat::RelativeMilliseconds),
          DataType::Time64(TimeUnit::Microsecond) =>
  Some(TimeFormat::RelativeMicroseconds),
          DataType::Time64(TimeUnit::Nanosecond) =>
  Some(TimeFormat::RelativeNanoseconds),
