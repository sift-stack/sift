...
pub enum ImportCmd {
    ...
    /// Import a PX4 ULog file into Sift. Timestamps are anchored by the log's
    /// GPS time fix, or by --relative-start-time when the log has none.
    Ulog(ImportUlogArgs),
...
pub struct ImportUlogArgs {
    ...
    /// Log-start time (RFC3339) used to anchor the file's boot-relative
    /// timestamps. Takes precedence over the log's GPS time fix; required if
    /// the log has no GPS fix
    #[arg(short = 's', long)]
    pub relative_start_time: Option<String>,

    /// Info message key to import as run metadata (stored as info.<key>); can
    /// be specified multiple times. Requires --run or --run-id
    #[arg(long)]
    pub info_key: Vec<String>,

    /// Parameter name to import as run metadata (stored as param.<name>); can
    /// be specified multiple times. Requires --run or --run-id
    #[arg(long)]
    pub param_key: Vec<String>,

    /// How to handle recoverable parse errors such as a truncated or corrupt
    /// record
