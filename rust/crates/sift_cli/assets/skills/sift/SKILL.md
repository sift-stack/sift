...
- **Query many runs, channels, or a long time range.** Probe first. Fetch one
  run or channel, or a short window of the range, and read the result before
  you issue the rest. The probe shows whether the selectors match and what a
  call costs. Run four calls or fewer in parallel. Split a long range into
  successive calls with disjoint `[start, end)` windows, because one long call
  can hit the client's request timeout and return nothing. Pass every resulting
  file to one `sql` query instead of reading them one at a time in a script.
- **Query a channel registration.** Pass `channel_id` or `channel_ids` to
  ...
  `empty_channels` lists raw channel IDs with no samples in the queried window.
  Calculated channels use their query keys instead. An empty registration does
  not mean other registrations with the same name are empty.
