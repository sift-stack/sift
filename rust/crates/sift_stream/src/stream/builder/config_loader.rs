...
mod tests {
    ...
    /// Roughly 5 MiB of flow schema, over tonic's 4 MiB default but under our 50 MiB one.
    fn create_oversized_flows() -> Vec<FlowConfig> {
        (0..300)
            .map(|flow| FlowConfig {
                name: format!("flow_{flow:04}"),
                channels: (0..400)
                    .map(|channel| ChannelConfig {
                        name: format!("channel_{flow:04}_{channel:04}_padded_to_add_bytes"),
                        data_type: ChannelDataType::Double.into(),
                        ..Default::default()
                    })
                    .collect(),
    ...
    #[tokio::test]
    async fn test_load_ingestion_config_decodes_flow_schema_above_tonic_default() {
        let (grpc_channel, _) = create_mock_grpc_channel_with_ingestion_service(
            MockIngestionConfigService::with_flows(create_oversized_flows()),
        )
        .await;

        // No local flows means an empty filter, so every flow is fetched.
        ...
        let (_, flows, _) = load_ingestion_config(grpc_channel, form, ServiceOptions::default())
            .await
            .expect("default options should decode a flow schema over tonic's 4 MiB default");

        assert_eq!(flows.len(), 300);
    }

    #[tokio::test]
    async fn test_load_ingestion_config_reports_flow_schema_over_decode_limit() {
        let (grpc_channel, _) = create_mock_grpc_channel_with_ingestion_service(
            MockIngestionConfigService::with_flows(create_oversized_flows()),
        )
        .await;
        ...
        let options = ServiceOptions::default().with_max_decoding_message_size(4 * 1024 * 1024);
        let err = load_ingestion_config(grpc_channel, form, options)
            .await
            .expect_err("a 4 MiB limit should reject a 5 MiB flow schema");

        assert_eq!(err.kind(), ErrorKind::RetrieveIngestionConfigError);
        let message = err.to_string();
        assert!(
            message.contains("larger than this client is configured to decode"),
            "expected the decode-limit message, got: {message}"
        );
        assert!(
            message.contains("max_decoding_message_size"),
            "expected remediation naming the setting, got: {message}"
        );
    }
