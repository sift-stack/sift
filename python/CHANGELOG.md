...
key = client.resource_attributes.get_or_create_key("psm_id", ResourceAttributeKeyType.SET_OF_ENUM)
...
key = client.principal_attributes.get_or_create_key("psm_id", PrincipalAttributeValueType.SET_OF_ENUM)
...
This replaces the previous setup that required `conftest.py` boilerplate and a `from sift_client.util.test_results import *` activation. A default `sift_client` fixture reads `SIFT_*` env vars with `sift_grpc_uri` / `sift_rest_uri` ini fallback, so per-project config lives in `pyproject.toml`. `sift_include` / `sift_exclude` markers and the `sift_autouse` ini key scope which tests produce Sift reports, and autouse fixtures are gated per test so mixed unit and integration suites share one `pytest` invocation without unit tests paying a backend round-trip.
...
#### Terminal summary
A session-start header shows the SDK version and active mode (e.g. `Sift: sift-stack-py 0.18.0 — online mode`). An end-of-run Sift report panel reports status, step and measurement counts (color-coded), test case, system, log file, and a clickable report link (online) or upload command (offline). New `--sift-open-report` / `sift_open_report` opens the report in a browser at session end on interactive terminals. `sift_app_url` / `SIFT_APP_URL` sets the web-app origin for on-prem and custom deployments; `SiftClient.app_url` exposes the resolved origin to non-plugin consumers.
...
- [pytest pass/fail behavior improvements](https://github.com/sift-stack/sift/pull/568)
- [Graceful handling of a missing connection](https://github.com/sift-stack/sift/pull/569)
...
- [pytest summary output](https://github.com/sift-stack/sift/pull/594)
- [Flexible report naming and consolidated options](https://github.com/sift-stack/sift/pull/602)
- [Exit instead of raise on connection failure](https://github.com/sift-stack/sift/pull/606)
- [Fix incremental upload bug and add unit tests](https://github.com/sift-stack/sift/pull/611)
- [Order-independent report tree with early parent close](https://github.com/sift-stack/sift/pull/616)
