"""gRPC caching interceptor for transparent local response caching.

This module provides a caching interceptor that can be used to cache gRPC unary-unary
responses locally using diskcache. The cache is persistent across runs and supports TTL
expiration and per-request control via metadata.

Usage:
    from sift_py.grpc._interceptors.caching import CachingInterceptor

    # Create interceptor with 1 hour TTL
    cache_interceptor = CachingInterceptor(ttl=3600, cache_path=".grpc_cache")
...
class CachingInterceptor(ClientInterceptor):
    """Interceptor that caches unary-unary gRPC responses locally.
    ...
    def __init__(
        ...
        logger.debug(
            f"Initialized CachingInterceptor with ttl={ttl}s, "
    ...
    def intercept(
        ...
        """Intercept the gRPC call and apply caching logic.
        ...
        if force_refresh:
            ...
            response = method(request_or_iterator, client_call_details)
