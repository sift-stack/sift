...
def test_report_template_service_get_report_template_by_id(report_template_service):
    ...
    with mock.patch.object(
        ReportTemplateService, "_get_report_template_by_id"
...
def test_report_template_service_get_report_template_missing_client_key_and_id(
    ...
    with pytest.raises(
        ValueError, match="Either client_key or report_template_id must be provided"
    ):
...
def test_report_template_service__get_rule_client_keys_missing_rule_client_key(
    report_template_service,
):
    report_template_config = ReportTemplateConfig(
        name="report-template",
    ...
    with pytest.raises(Exception):
        report_template_service._get_rule_client_keys(
            report_template_config, match="rule rule-2 requires a rule_client_key"
        )
