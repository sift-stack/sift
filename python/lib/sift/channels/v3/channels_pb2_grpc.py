# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc

from sift.channels.v3 import channels_pb2 as sift_dot_channels_dot_v3_dot_channels__pb2


class ChannelServiceStub(object):
    """Missing associated documentation comment in .proto file."""

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.GetChannel = channel.unary_unary(
                '/sift.channels.v3.ChannelService/GetChannel',
                request_serializer=sift_dot_channels_dot_v3_dot_channels__pb2.GetChannelRequest.SerializeToString,
                response_deserializer=sift_dot_channels_dot_v3_dot_channels__pb2.GetChannelResponse.FromString,
                )
        self.ListChannels = channel.unary_unary(
                '/sift.channels.v3.ChannelService/ListChannels',
                request_serializer=sift_dot_channels_dot_v3_dot_channels__pb2.ListChannelsRequest.SerializeToString,
                response_deserializer=sift_dot_channels_dot_v3_dot_channels__pb2.ListChannelsResponse.FromString,
                )


class ChannelServiceServicer(object):
    """Missing associated documentation comment in .proto file."""

    def GetChannel(self, request, context):
        """Retrieve a channel
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def ListChannels(self, request, context):
        """Retrieve channels using an optional filter.
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_ChannelServiceServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'GetChannel': grpc.unary_unary_rpc_method_handler(
                    servicer.GetChannel,
                    request_deserializer=sift_dot_channels_dot_v3_dot_channels__pb2.GetChannelRequest.FromString,
                    response_serializer=sift_dot_channels_dot_v3_dot_channels__pb2.GetChannelResponse.SerializeToString,
            ),
            'ListChannels': grpc.unary_unary_rpc_method_handler(
                    servicer.ListChannels,
                    request_deserializer=sift_dot_channels_dot_v3_dot_channels__pb2.ListChannelsRequest.FromString,
                    response_serializer=sift_dot_channels_dot_v3_dot_channels__pb2.ListChannelsResponse.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'sift.channels.v3.ChannelService', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))


 # This class is part of an EXPERIMENTAL API.
class ChannelService(object):
    """Missing associated documentation comment in .proto file."""

    @staticmethod
    def GetChannel(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/sift.channels.v3.ChannelService/GetChannel',
            sift_dot_channels_dot_v3_dot_channels__pb2.GetChannelRequest.SerializeToString,
            sift_dot_channels_dot_v3_dot_channels__pb2.GetChannelResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def ListChannels(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/sift.channels.v3.ChannelService/ListChannels',
            sift_dot_channels_dot_v3_dot_channels__pb2.ListChannelsRequest.SerializeToString,
            sift_dot_channels_dot_v3_dot_channels__pb2.ListChannelsResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)
