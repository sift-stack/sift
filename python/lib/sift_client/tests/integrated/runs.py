...
async def main():
    ...
    # 2m: Test pagination
    print("\n   2m. Testing pagination...")
    page_size = 2
    runs = client.runs.list(limit=page_size)
    print(f"   First page ({len(runs)} runs):")
    for run in runs:
        print(f"   - {run.name}")

    if len(runs) == page_size:
        # Get next page
        next_page_token = runs._response.next_page_token if hasattr(runs, "_response") else None
        if next_page_token:
            runs_page2 = client.runs.list(limit=page_size, page_token=next_page_token)
            print(f"   Second page ({len(runs_page2)} runs):")
            for run in runs_page2:
                print(f"   - {run.name}")
        else:
