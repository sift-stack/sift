...
class Channel(BaseType[ChannelProto, "Channel"]):
    ...
    def _from_proto(
        ...
        if isinstance(proto, ChannelProto):
            return cls(
                ...
                enum_types=cls._proto_list_to_dict(proto.enum_types),  # type: ignore
    ...
    def to_rust_config(self) -> ChannelConfigPy:
        return ChannelConfigPy(
            name=self.name,
            data_type=self.data_type.to_rust_type(),
            ...
            enum_types=[
                ...
                for enum_name, enum_key in self.enum_types.items()
            ]
            if self.enum_types
            else [],
        )

    def _rust_channel_value_from_bitfield(self, value: Any) -> ChannelValuePy:
        ...
        """
        assert self.bit_field_elements is not None
        ...
        if len(list_value) != len(self.bit_field_elements):
            raise ValueError(
                f"Expected number of values passed as list to match number of bit field elements  for {self.name}, but got {len(list_value)}"
            )
    ...
    def to_rust_value(self, value: Any) -> ChannelValuePy:
        ...
        if self.data_type == ChannelDataType.ENUM:
            enum_name = value
            enum_val = self.enum_types.get(enum_name)
            if enum_val is None:
            ...
            return ChannelValuePy.enum_value(self.name, ChannelEnumTypePy(enum_name, enum_val))
        elif self.data_type == ChannelDataType.BIT_FIELD:
            return self._rust_channel_value_from_bitfield(value)
...
class Flow(BaseType[FlowConfig, "Flow"]):
    ...
    def to_rust_config(self) -> FlowConfigPy:
        return FlowConfigPy(
            name=self.name,
            channels=[channel.to_rust_config() for channel in self.channels],
        )
