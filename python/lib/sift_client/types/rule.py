...
class Rule(BaseType[RuleProto, "Rule"]):
    ...
    def create(
        ...
        rule.client.rules.create_from_model(rule=rule)
        return rule
    ...
    def _from_proto(cls, proto: RuleProto, sift_client: SiftClient | None = None) -> Rule:
        return cls(
            ...
            rule_version=RuleVersion._from_proto(proto.rule_version)
            if proto.rule_version
            else None,
            client_key=proto.client_key if proto.client_key else None,
            asset_configuration=proto.asset_configuration,
            contextual_channels=proto.contextual_channels,
...
class RuleUpdate(ModelUpdate[RuleProto]):
    ...
    conditions: List[UpdateConditionRequest] | None = None
...
class RuleCondition(BaseModel):
    ...
    rule_condition_version_id: str

    @classmethod
    def _from_proto(cls, proto) -> RuleCondition:
        return cls(
...
class RuleAction(BaseModel):
    ...
    created_by_user_id: str
    modified_by_user_id: str
    rule_action_version_id: str
    annotation_type: RuleAnnotationType = RuleAnnotationType.UNSPECIFIED
    notify_recipients: Optional[List[str]] = None  # List of user IDs to notify
    ...
    def _from_proto(cls, proto: RuleActionProto) -> RuleAction:
        return cls(
            ...
            tags=list(proto.configuration.annotation.tag_ids)
            if proto.configuration.annotation.tag_ids
            else None,
            assignee=proto.configuration.annotation.assigned_to_user_id
            if proto.configuration.annotation.assigned_to_user_id
            else None,
            type=RuleActionType.from_str(proto.action_type),
        )
...
class ExpressionChannelReferenceChannelConfig(BaseModel):
    """
    `channel_reference`: The channel reference (e.g. '$1') used in the expression.
    `channel_config`: Instance of `ChannelConfig`.
    """
