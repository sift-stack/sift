...
class TestAnnotations:
    ...
    def test_get(self, sift_client, new_annotation):
        ...
        fetched = sift_client.annotations.get(annotation_id=new_annotation._id_or_error)
    ...
    def test_update(self, sift_client, new_annotation):
        ...
        updated = sift_client.annotations.update(
            new_annotation,
            AnnotationUpdate(description="updated", state=AnnotationState.FAILED),
        )

        assert updated.description == "updated"
        assert updated.state is AnnotationState.FAILED
    ...
    def test_instance_methods(self, sift_client, test_timestamp_str, window, nostromo_asset):
        ...
        annotation.set_accepted()
        assert annotation.state is AnnotationState.ACCEPTED
...
class TestAnnotationLogs:
    ...
    def test_comment(self, sift_client, new_annotation):
        ...
        log = sift_client.annotations.logs.add_comment(new_annotation, "looks fine to me")
    ...
    def test_list_filtered_by_kind(self, sift_client, new_annotation):
        ...
        sift_client.annotations.logs.add_comment(new_annotation, "another comment")
    ...
    def test_annotation_logs_property(self, sift_client, new_annotation):
        ...
        sift_client.annotations.logs.add_comment(new_annotation, "via property")
...
class TestAnnotationWorkflow:
    ...
    def test_create_derives_the_asset_from_channels(
        self, sift_client, test_timestamp_str, window, nostromo_asset, nostromo_channels
    ):
        """Passing Channel objects links them and fills in the asset."""
        start, end = window
        review = sift_client.annotations.create(
            AnnotationCreate(
                name=f"test_review_{test_timestamp_str}",
                start_time=start,
                end_time=end,
                linked_channels=nostromo_channels,
    ...
    def test_create_phase_model(
        self, sift_client, test_timestamp_str, window, nostromo_asset, nostromo_channels
    ):
        """PhaseCreate makes a phase with no review state."""
        start, end = window
        phase = sift_client.annotations.create(
            PhaseCreate(
                name=f"test_phase_verb_{test_timestamp_str}",
                start_time=start,
                end_time=end,
                linked_channels=nostromo_channels,
            )
    ...
    def test_create_with_explicit_assets(
        ...
        """`assets` accepts Asset objects and skips the channel lookup."""
        start, end = window
        review = sift_client.annotations.create(
            AnnotationCreate(
                name=f"test_review_assets_{test_timestamp_str}",
                start_time=start,
                end_time=end,
    ...
    def test_state_verbs(self, sift_client, new_annotation):
        """Each review verb changes the state."""
        assert (
            sift_client.annotations.set_accepted(new_annotation).state is AnnotationState.ACCEPTED
        )
        assert sift_client.annotations.set_failed(new_annotation).state is AnnotationState.FAILED
        assert sift_client.annotations.set_open(new_annotation).state is AnnotationState.OPEN

    def test_instance_state_verbs(self, sift_client, new_annotation):
        """Test the review verbs on the Annotation instance."""
        assert new_annotation.set_accepted().state is AnnotationState.ACCEPTED
        assert new_annotation.set_failed().state is AnnotationState.FAILED
        assert new_annotation.set_open().state is AnnotationState.OPEN

    def test_instance_comment(self, sift_client, new_annotation):
        """Test commenting from the Annotation instance."""
        log = new_annotation.add_comment("from the instance")
